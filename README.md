...
- **图片自动撤回** —— 可选在设定时间后自动撤回本插件发送的所有图片（OneBot v11）
...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `auto_withdraw_enabled` | bool | `false` | 启用图片自动撤回：开启后本插件发送的所有图片会在设定时间后自动撤回（当前仅 OneBot v11 / aiocqhttp 平台支持） |
| `auto_withdraw_seconds` | int | `60` | 自动撤回延时（秒，范围 5–3600） |

> 自动撤回开启时，OneBot 平台会改为逐张发送图片（不再合并转发），以便每张图片都能单独撤回；其他平台自动忽略该功能。
