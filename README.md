...
版本：`2.2.2`  ·  兼容 AstrBot：`>=4.9.2`  ·  依赖：`httpx>=0.24`、`Pillow>=9.0`
...
- **消息自动撤回** —— 可选开启：插件发送的消息在设定时间后自动撤回（仅 OneBot v11）
...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `auto_recall_enabled` | bool | `false` | 自动撤回插件发送的消息（仅 OneBot v11 生效） |
| `auto_recall_delay` | int | `60` | 消息发送后经过该秒数自动撤回（范围 3–3600） |

> 群消息撤回需要机器人在该群拥有管理员权限；私聊消息撤回不受此限制。
